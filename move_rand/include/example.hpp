...
public:
    ...
    void upload(const void * data, int width, int height, rs::format format)
    {
        ...
        case rs::format::z16:
            ...
            uint16_t *p;
            p = (uint16_t*)(data);
            if (p[300*480 + 400] == 0) break;
            freopen("aa.txt", "a+", stdout);
            for (int i = 0; i < width; ++i) {
                for (int j = 0; j < height; ++j) {
                    printf("%u ", p[i*height + j]);
                }
                printf("\n");
            }
            printf("\n");
        ...
        case rs::format::rgb8: case rs::format::bgr8: // Display both RGB and BGR by interpreting them RGB, to show the flipped byte ordering. Obviously, GL_BGR could be used on OpenGL 1.2+
            /*uint16_t *pp;
            pp = (uint16_t*)(data);
            if (pp[300*480 + 400] == 0) break;
            freopen("aa1.txt", "a+", stdout);
            for (int i = 0; i < width; ++i) {
                for (int j = 0; j < height; ++j) {
                    printf("%u ", pp[i*height + j]);
                }
                printf("\n");
            }
            printf("\n");
    ...
    const void* upload(rs::device & dev, rs::stream stream)
    {
        ...
        {
            data = dev.get_frame_data(stream);
            upload(data, dev.get_stream_width(stream), dev.get_stream_height(stream), dev.get_stream_format(stream));
    ...
    const void* show(rs::device & dev, rs::stream stream, int rx, int ry, int rw, int rh)
    {
        const void* data = NULL;
        if(!dev.is_stream_enabled(stream)) return data;

        data = upload(dev, stream);
