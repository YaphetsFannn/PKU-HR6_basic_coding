...
void poseMessageReceived(const naoqi_msgs::JointAnglesWithSpeed& msg) {
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        float phi1 = ((float)rand() / RAND_MAX - 0.5) * 360.0 * acos(-1.0) / 180;
        float phi2 = ((float)rand() / RAND_MAX - 0.5) * 360.0 * acos(-1.0) / 180;
        float phi3 = ((float)rand() / RAND_MAX - 0.5) * 360.0 * acos(-1.0) / 180;
        ...
        output_tmp.clear();
        output_tmp = calculate_PFM(input_tmp);
        //Init();
        //hr6_arm_fwd_kinematics(output);
        tmp.clear();
        del_t = 0;
