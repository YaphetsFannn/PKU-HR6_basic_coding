...
void InitNetI(NN *n) {
    ...
    string path[NETNUM_I] = { path_prefix + "IM.txt"};
...
void InitNetI1(NN *n) {
	string path_prefix = "/home/pku-hr6/catkin_ws/src/network/";
    string path[NETNUM_I] = { path_prefix + "hIM1.txt"};
       for (int i = 0; i < NETNUM_I; ++i) {
        n[i] = NN(path[i]);
        //cout << path[i] << endl;
	}
...
void InitNetI2(NN *n) {
    string path_prefix = "/home/pku-hr6/catkin_ws/src/network/";
    string path[NETNUM_I] = { path_prefix + "hIM2.txt"};
       for (int i = 0; i < NETNUM_I; ++i) {
        n[i] = NN(path[i]);
        //cout << path[i] << endl;
    }
...
void InitNetI3(NN *n) {
    string path_prefix = "/home/pku-hr6/catkin_ws/src/network/";
    string path[NETNUM_I] = { path_prefix + "hIM3.txt"};
       for (int i = 0; i < NETNUM_I; ++i) {
        n[i] = NN(path[i]);
        //cout << path[i] << endl;
    }
...
void InitNetI4(NN *n) {
    string path_prefix = "/home/pku-hr6/catkin_ws/src/network/";
    string path[NETNUM_I] = { path_prefix + "hIM4.txt"};
       for (int i = 0; i < NETNUM_I; ++i) {
        n[i] = NN(path[i]);
        //cout << path[i] << endl;
    }
...
void InitNetI5(NN *n) {
    string path_prefix = "/home/pku-hr6/catkin_ws/src/network/";
    string path[NETNUM_I] = { path_prefix + "hIM5.txt"};
       for (int i = 0; i < NETNUM_I; ++i) {
        n[i] = NN(path[i]);
        //cout << path[i] << endl;
    }
...
void InitNetI6(NN *n) {
    string path_prefix = "/home/pku-hr6/catkin_ws/src/network/";
    string path[NETNUM_I] = { path_prefix + "hIM6.txt"};
       for (int i = 0; i < NETNUM_I; ++i) {
        n[i] = NN(path[i]);
        //cout << path[i] << endl;
    }
...
void InitNetF(NN *n) {
    string path_prefix = "/home/pku-hr6/catkin_ws/src/network/";
    string path[NETNUM_F] = { path_prefix + "FM.txt"};
...
void poseMessageReceived(const naoqi_msgs::JointAnglesWithSpeed& msg) {
    ...
    //input[11] += off_set - 3;
    //input[10] -= 2;
    //if (cuboid == 3) input[11] += 3.5;
    //if (input[11] < 12) input[11] = 7.5;
    //for (int i = 0; i < INPUTSIZE_I; ++i);
        //printf("%f ", input[i]);
    //puts("");
    ...
    for(int i = 1; i <= 6; ++i)
    {
        input_tmp = Forecast(nI[i], input, INPUTSIZE_I + i - 1, 1, NETNUM_I, normRangeI);
        output[i - 1] = input_tmp[0];
        input.push_back(output[i - 1]);
    }
    /*if(output[0] > 0)
    {
        puts("Too high, I can't grasp this object, sorry!");
        aux.data = 0;
...
int main(int argc, char **argv) {
    ...
    float arr[] = {.0664730000000000,	-0.143172000000000,	-0.747893000000000,	-0.539854000000000,	0.374854000000000,	-0.630546000000000,	0.420592000000000,	-0.652314000000000,	0.135685000000000,	-0.684997000000000,	-0.715799000000000,	0.188396000000000,	-1.24170200000000,	14.8182720000000};
    float arr1[] = {0.0664730000000000,	-0.143172000000000,	-1.84077700000000,	-0.429515000000000,	0.962999312998582,	-1.02265400000000};
