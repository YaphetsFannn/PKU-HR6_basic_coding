...
int main(int argc, char **argv) {
    ...
    FILE *fw_I, *fw_F;
    fw_I = fopen("data_I.txt", "a+");
    fw_F = fopen("data_F.txt", "a+");
    for(int t = 0; t < 50000; ++t) {
        //arr with dimension 8, H-N-Q
        vector<float> input_I, input_F;

        float arr[INPUTSIZE_F] = {0};
        for(int i = 0; i < INPUTSIZE_F; ++i)
            arr[i] = 1.0 * rand() / RAND_MAX * (normRangeF[i][1] - normRangeF[i][0]) + normRangeF[i][0];

        for(int i = 0; i < INPUTSIZE_F; i++)
            input_F.push_back(arr[i]);
        vector<float> output_F = calculate_PFM(input_F);
        for(int i = 0; i < INPUTSIZE_F; ++i)
            fprintf(fw_F, "%f ", input_F[i]);
        for(int i = 0; i < OUTPUTSIZE_F; ++i)
            fprintf(fw_F, "%f ", output_F[i]);
        fprintf(fw_F, "\n");

        for(int i = 0; i < 2; ++i)
            input_I.push_back(arr[i]);
        for(int i = 2; i < 14; i++)
            input_I.push_back(output_F[i - 2]);
        vector<float> output_I = calculate_hPIM(input_I);
        for(int i = 0; i < INPUTSIZE_I; ++i)
            fprintf(fw_I, "%f ", input_I[i]);
        for(int i = 0; i < OUTPUTSIZE_I; ++i)
            fprintf(fw_I, "%f ", output_I[i]);
        fprintf(fw_I, "\n");
    }
    fclose(fw_I);
    fclose(fw_F);
